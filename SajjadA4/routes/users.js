        ...
        var users = doesExist.count("", function(err, c){
          ...
          var fileToRead = pic.size > 0 ? pic.path : (path.join(__dirname, '../') + 'public/images/avatar.jpg');
            fs.readFile(fileToRead, function (err, data) {
              if (err) throw err;
              var img = {data: data, contentType: pic.type};
              // Save binary image.
              var profilePicture = mongoose.model('Picture').create({
                img: img   
    ...
    mongoose.model('User').findById(req.id, function (err, user) {
      if (err) {
        ...
        getUserRole(req, function(err, role) {
          ...
          var profilePic = mongoose.model('Picture').findById(user.profilePicture, function (err, picture) {
            if (err) {
              console.log('GET Error: There was a problem retrieving: ' + err);
            } 
